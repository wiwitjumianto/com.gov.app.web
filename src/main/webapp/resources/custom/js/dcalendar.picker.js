	...
	DCalendar.prototype = {
		...
		create : function(mode){
			...
			if(mode === "days") {
				...
				for(var i = 1; i <= 6; i++){
					 temp = [$('<td></td>'),$('<td></td>'),$('<td></td>'),$('<td></td>'),$('<td></td>'),$('<td></td>'),$('<td></td>')];
					...
					while(day <= days){
						...
						if(d.getTime() == today.getTime()) temp[dayOfWeek].attr('id', 'currDay');
                                                temp[dayOfWeek].attr('onclick', 'dateFunction(event)');
						...
						else if(dayOfWeek < 6) temp[dayOfWeek].html('<span>'+(day++)+'</span>').addClass('date dateSelector');
						else {
							temp[dayOfWeek].html('<span>'+(day++)+'</span>').addClass('date dateSelector');
							break;
						}
					}
						...
						if (i === 1) {
							...
							for (var a = 6; a >= 0; a--) {
								if (temp[a].text() === ''){
									...
									p.setDate(pDays);
                                                                        temp[a].attr('onclick', 'dateFunction(event)');
									temp[a].html('<span>' + (pDays--) + '</span>').addClass('pMDate dateSelector');
							...
							for (var a = 0; a <= 6; a++) {
								if (temp[a].text() === ''){
									...
									nextMonth.setDate(nStartDate);
                                                                        temp[a].attr('onclick', 'dateFunction(event)');
									temp[a].html('<span>' + (nStartDate++) + '</span>').addClass('nMDate dateSelector');
